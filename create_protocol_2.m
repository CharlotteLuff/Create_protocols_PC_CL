...
s1 = {'2_df5_1ep' ,'2_df10_1ep','2_df40_1ep'}; % define df
s2 = {'2_df5_2ep' ,'2_df10_2ep','2_df40_1ep'}; % define df

%% Stimulation signal script

% User input

f = [10 100 500 1000 2500 5000];
range = 9;

for cf = 1:length(f)
    for x = 1: length(df)
        rand_f1.(char(sf(x))) = randi([f(cf)-range f(cf)+range]);
        rand_f2.(char(sf(x))) =  rand_f1.(char(sf(x))) + df(x);
        rand_pair.(char(sf(x)))(cf,:) = [rand_f1.(char(sf(x))) rand_f2.(char(sf(x)))];
    end
end
random_freq_pairs_2 = rand_pair(randperm(size(rand_pair, 1)), :);

two_five_hz = random_freq_pairs_2.df5;
two_ten_hz = random_freq_pairs_2.df10;
two_forty_hz = random_freq_pairs_2.df40;

save('2_5hz', 'two_five_hz');
save('2_10hz', 'two_ten_hz');
save('2_40hz', 'two_forty_hz');

%% Create waveforms

% 1 electrode pair - protocol 2
...
for i = 1:length(f)
    for x = 1:length(df)
        ...
        dt = 0.004;
        f1.(char(sf(x)))(i,1) = random_freq_pairs_2.(char(sf(x)))(i,1);
        f2.(char(sf(x)))(i,1) = random_freq_pairs_2.(char(sf(x)))(i,2);
...
%         I2_stim.(char(sf(x)))(i,:) = A2*cos(2*pi*f2.(char(sf(x)))(i,1)*stim_tt+phi2);
%         
%         if ramp_up_t
%             ramp_up_size = round(ramp_up_t/dt);
%             ramp_vec = 0:1/ramp_up_size:1;
%             
%              I1_stim.(char(sf(x)))(1:length(ramp_vec)) = ramp_vec.*I1_stim.(char(sf(x)))(1:length(ramp_vec));
%              I2_stim.(char(sf(x)))(1:length(ramp_vec)) = ramp_vec.*I1_stim.(char(sf(x)))(1:length(ramp_vec));
%         end
%         
%         if ramp_down_t
%             ramp_down_size = round(ramp_down_t/dt);
%             ramp_vec = fliplr(0:1/ramp_down_size:1);
%             
%              I1_stim.(char(sf(x)))(end-length(ramp_vec)+1:end) = ramp_vec.*I1_stim.(char(sf(x)))(end-length(ramp_vec)+1:end);
%              I2_stim.(char(sf(x)))(end-length(ramp_vec)+1:end) = ramp_vec.*I1_stim.(char(sf(x)))(end-length(ramp_vec)+1:end);
%         end
%         
